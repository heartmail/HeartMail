-- Add custom variables support to recipients table
-- This will allow users to store custom fields for each recipient

-- Add custom_variables column to recipients table
ALTER TABLE recipients 
ADD COLUMN IF NOT EXISTS custom_variables JSONB DEFAULT '{}';

-- Create index for better performance on custom variables queries
...
-- Create function to validate custom variable names
CREATE OR REPLACE FUNCTION is_valid_custom_variable_name(var_name TEXT)
RETURNS BOOLEAN AS $$
BEGIN
    -- Check if variable name is valid (alphanumeric + underscore, not starting with number)
    RETURN var_name ~ '^[a-zA-Z_][a-zA-Z0-9_]*$' AND length(var_name) <= 50;
END;
$$ LANGUAGE plpgsql;

-- Create function to get custom variable value for a recipient
CREATE OR REPLACE FUNCTION get_recipient_custom_variable(recipient_id_param UUID, var_name TEXT)
RETURNS TEXT AS $$
DECLARE
    var_value TEXT;
BEGIN
